...
public interface BaseGetByIdDeploymentRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseGetByIdDeploymentRestResource.class);
    ...
    @MethodStats
    default ResponseEntity<Mono<DeploymentDto>> getById(@PathVariable("id") String id, ServerWebExchange serverWebExchange, Principal principal) throws JsonProcessingException {
        log.debug("REST request to get one deployment by id, id {}", id);
    ...
    default ResponseEntity<Mono<DeploymentDto>> getByIdResponse(Mono<DeploymentDto> result, Optional<USER> user) {
